...
export function renderString<T extends ParseType>(
  ...
  return spec.raw.pipe(
    ...
    switchMap((el) => {
      ...
      return merge(node$, merge(...obsList).pipe(switchMapTo(EMPTY)));
    }),
  );
}
...
function createExtraBindingObjects<O extends ExtraSpec>(
  ...
  for (const key in specs) {
    ...
    partial[key] = createExtraBindings(spec, target, context);
  }
