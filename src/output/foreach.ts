...
export class ResolvedOForeach<T> implements OForeach<T> {
  ...
  resolve(
    ...
    renderFn: RenderValueFn<T>,
  ) => OperatorFunction<readonly T[], readonly T[]> {
    return (renderFn: RenderValueFn<T>) => {
      const slotEl$ = of(target).pipe(initSlot(this.slotName));
          ...
          tap(([newNodes, slotNode]) => {
            ...
            while (diffs.length > 0) {
              ...
              switch (diff.type) {
                ...
                case 'delete': {
                  const currentNode = currentNodes[diff.index];
                  if (!currentNode) {
