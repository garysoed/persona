...
import {Environment, fake, runEnvironment, spy} from 'gs-testing';
...
export class Tester {
  ...
  bootstrapElement<E extends HTMLElement, S extends Spec>(
    ...
    const element = this.customElementRegistry.create(
      spec.namespace,
      spec.tag,
    ) as E & DecoratedElement;
  ...
  setMedia(query: string, value: boolean): void {
    ...
    if (!(mediaQuery instanceof FakeMediaQuery)) {
      throw new Error(
        `mediaQuery should be a ${FakeMediaQuery} but was ${mediaQuery}`,
      );
    }
...
export function setupTest(spec: TestSpec): Tester {
  ...
  const tester = new Tester(vine, customElementRegistry);
...
function recursiveUpgradeNode(
  node: Node,
  fakeRegistry: FakeCustomElementRegistry,
): void {
