...
export function query<S extends Spec, X extends ExtraBindingSpec<Element>>(
  ...
  const reversedSpecs = reverseSpec(registration?.spec.host ?? {});
  for (const key in reversedSpecs) {
    providers[key] = (root: Target, context: RenderContext) => {
      const spec = reversedSpecs[key];
      if (!spec) {
        throw new Error(`No specs for key ${key} found`);
      }
      return spec.resolve(getElement(root, query), context);
    };
  }

  const normalizedExtra: Record<
    string,
    InputOutputThatResolvesWith<Element>
  > = extra ?? {};
  for (const key in normalizedExtra) {
    providers[key] = (root: Target, context: RenderContext) => {
      const spec = normalizedExtra[key];
      if (!spec) {
        throw new Error(`No specs for key ${key} found`);
      }
      return spec.resolve(getElement(root, query), context);
  ...
  return providers as ResolvedBindingSpecProvider<
    ReversedSpec<S['host'] & {}> & X,
    Element
  >;
}
