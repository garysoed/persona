import {WindowLike} from 'gs-testing/export/fake/dom';
...
export class LocationService<S extends RouteSpec> {
  ...
  getLocationOfType<K extends keyof S>(
    type: K,
  ): Observable<Route<S, K> | null> {
  ...
  goToPath<K extends keyof S>(type: K, payload: Payloads<S>[K]): void {
    const url =
      this.getUrl(type, payload) ||
      this.getUrl(this.defaultRoute.type, this.defaultRoute.payload);
    if (!url) {
      throw new Error(`Invalid route: ${JSON.stringify({payload, type})}`);
    }
  ...
  private getUrl<K extends keyof S>(
    type: K,
    payload: Payloads<S>[K],
  ): string | null {
    const spec = this.specs[type];
    if (!spec) {
      throw new Error(`No spec for key ${String(type)} found`);
    }
    const result = spec.convertBackward(payload);
    if (!result.success) {
      return null;
    }
  ...
  private parseLocation(location: string): Route<S, keyof S> | null {
    for (const specKey of Object.keys(this.specs) as Array<keyof S>) {
      ...
      if (!spec) {
        throw new Error(`No spec for key ${String(specKey)} found`);
      }
      const result = spec.convertForward(location) as Result<
        Payloads<S>[keyof S]
      >;
      if (result.success) {
        return {
