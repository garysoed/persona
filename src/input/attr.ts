  ...
  resolve(target: Element): Observable<T | null> {
    ...
    return defer(() => {
      ...
      mapForward(
        ...
        (value) =>
          throwError(`Invalid value of attribute ${this.attrName}: ${value}`),
      ),
    );
  }
}
...
export function iattr<T = never>(
  attrName: string,
  converter: Converter<string, T>,
): ResolvedIAttr<T>;
export function iattr(
  attrName: string,
  converter?: Converter<string, unknown>,
): ResolvedIAttr<any> {
  if (converter) {
